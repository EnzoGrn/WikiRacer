...
export default function DailyPage() {
  ...
  const [gameState, setGameState] = useState<DailyState>({
    status: 'idle',
    clicks: 0,
    path: [],
    startedAt: null,
  ...
  if (gameState.status === 'finished') {
    const time = gameState.finishedAt! - gameState.startedAt!;
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-8 p-8">
        ...
        <div className="flex gap-8 text-center">
          ...
          <div>
            <p className="text-3xl font-bold">{minutes}:{seconds}</p>
    ...
    <main className="flex h-screen overflow-hidden">
      ...
      <aside className="w-64 flex-shrink-0 border-r flex flex-col gap-4 p-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-gray-400">{new Date(route.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
